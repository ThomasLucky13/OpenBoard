...
QList<QPointF> UBGraphicsVectorItem::linePoints()
{
    ...
    QLineF newLine = line();
    newLine.setP1(QPointF(line().x1()+mStrokeGroup->transform().dx(),
                          line().y1()+mStrokeGroup->transform().dy()));
    newLine.setP2(QPointF(line().x2()+mStrokeGroup->transform().dx(),
                          line().y2()+mStrokeGroup->transform().dy()));
    newLine.setAngle(line().angle() + qRadiansToDegrees(qAtan2(mStrokeGroup->transform().m12(), mStrokeGroup->transform().m11())));

    qreal incr = 1/newLine.length();
    if (incr<0) incr*=-1;
    if (incr>0)
    {
       for (qreal t = 0; t <= 1; t+=incr)
       {
           points.push_back(newLine.pointAt(t));
       }
    }
    return points;
}
